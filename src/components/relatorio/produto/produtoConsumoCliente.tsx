...
interface State {
  paginaCliente: number; // índice do cliente atual
}

export default class ProdutoConsumoCliente extends Component<Props, State> {
  state: State = {
    paginaCliente: 0,
  };
  ...
  proximoCliente = () => {
    this.setState((prev) => ({
      paginaCliente:
        prev.paginaCliente < this.clientes.length - 1
          ? prev.paginaCliente + 1
          : prev.paginaCliente,
    }));
  };
  ...
  clienteAnterior = () => {
    this.setState((prev) => ({
      paginaCliente: prev.paginaCliente > 0 ? prev.paginaCliente - 1 : 0,
    }));
  };

  render() {
    const { show, onHide } = this.props;
    const { paginaCliente } = this.state;

    const clienteAtual = this.clientes[paginaCliente];
    ...
    return (
      <Modal show={show} onHide={onHide} centered size="lg">
        ...
        <Modal.Footer style={{ backgroundColor: "rgb(255, 161, 106)" }}>
          ...
          <Button
            style={{ backgroundColor: "rgb(69,32,23)", borderColor: "rgb(69,32,23)" }}
            onClick={this.proximoCliente}
            disabled={paginaCliente === this.clientes.length - 1}
          >
            Próximo ➡
          </Button>
        </Modal.Footer>
